...
public class RequestGame extends JDialog {
			...
			getContentPane().add(buttonPane, BorderLayout.SOUTH);
			{
				...
				okButton.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						Library.requestList.addGame(textField_name.getText().trim(), textField_genre.getText().trim(),
								textField_releaseDate.getText().trim(), textField_platform.getText().trim(),
								textField_company.getText().trim());
						Library.requestList.save();
						dispose();
					}
				});
